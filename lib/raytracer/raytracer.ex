defmodule MyElixirRayTracer.Raytracer do
  ...
  @wall_z   100 # z of the wall on which the sphere is projected
  @canvas_w 100 # width of the canvas
  @canvas_h 100 # height of the canvas

  def trace() do
    canvas = canvas(@canvas_w, @canvas_h, color(1, 1, 1))
    eye_position = point(0, 0, -100)
    sphere_trans = identity_matrix4x4() |> scaling(20, 20, 20)
  ...
  defp scan(x, y, eye_position, canvas, canvas_trans, sphere) do
    ...
    cond do
      ...
      x >= canvas.width / 2 and y > -canvas.height / 2 -> scan(-canvas.width / 2, y-1, eye_position, canvas, canvas_trans, sphere)
